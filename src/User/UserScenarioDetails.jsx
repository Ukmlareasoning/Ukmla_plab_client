...
// Mock: scenario details with lectures and questions (attempt history with date, right/wrong)
const getScenarioDetailsByScenarioId = (scenarioId) => {
  const baseLectures = [
    {
      id: 1,
      lectureNo: 1,
  ...
  return {
    courseTitle: 'Scenario Title',
    lectures: baseLectures,
    coursePercentage,
  }
}

function UserScenarioDetails() {
  ...
  const courseTitle = course?.title || location.state?.courseTitle || 'Scenario Details'

  const details = getScenarioDetailsByScenarioId(courseId)
  ...
  const initialStaticRatings = useMemo(() => {
    ...
    if (exam2) {
      obj[`${courseId}-${exam2.id}`] = {
        stars: 4,
        comment: 'Very helpful for exam preparation. Clear structure and relevant scenarios.',
      }
    }
    return obj
  }, [courseId, lectures])
  ...
  const handleBack = () => {
    navigate('/user-dashboard/scenarios-history')
  }
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap', mb: 3 }}>
          <IconButton
            ...
            aria-label="Back to scenarios history"
          >
          ...
          <Box sx={{ flex: 1, minWidth: 0 }}>
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.25 }}>
              View scenario exams, questions, and your scores
            </Typography>
          </Box>
        </Box>
        ...
        {lectures.length > 0 ? (
          ...
          >
            ...
            <Typography variant="body1" color="text.secondary">
              No lectures or attempt history available for this scenario exam.
            </Typography>
          </Paper>
        )}
      </Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Box sx={{ minWidth: 0 }}>
              <Typography variant="h6" sx={{ fontWeight: 700, color: 'text.primary' }}>
                {ratingDialogMode === 'rate' ? 'Rate this scenario exam' : 'Your rating'}
              </Typography>
              {selectedExam && (
                <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.25 }}>
                  {selectedExam.courseTitle} – Scenario exam {selectedExam.lectureNo}
                </Typography>
              )}
            </Box>
          </Box>
            ...
            <Box>
              {selectedExam && getRatingForExam(`${courseId}-${selectedExam.lectureId}`) ? (
                ...
                <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                  You haven&apos;t rated this scenario exam yet. Use &quot;Rate this&quot; to add your rating and comment.
                </Typography>
              )}
            </Box>
          )}
...
export default UserScenarioDetails
