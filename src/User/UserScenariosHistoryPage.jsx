...
export default function UserScenariosHistoryPage() {
  ...
  // First 2 scenarios: show "Rate this" only. Other 2: show "View rate" only with static rating.
  const rateThisIds = useMemo(() => new Set(completedScenarios.slice(0, 2).map((c) => c.id)), [completedScenarios])
  const viewRateIds = useMemo(() => new Set(completedScenarios.slice(2, 4).map((c) => c.id)), [completedScenarios])

  const initialStaticRatings = useMemo(() => {
    const ids = completedScenarios.slice(2, 4).map((c) => c.id)
    const obj = {}
    ids.forEach((id, i) => {
      obj[id] = {
        stars: i === 0 ? 4 : 5,
        comment: i === 0
          ? 'Very helpful for exam preparation. Clear structure and relevant scenarios.'
          : 'Excellent scenario pack. Good mix of difficulty and topics.',
      }
    })
    return obj
  }, [completedScenarios])
  ...
  const handleSubmitRating = () => {
    ...
    setSavedRatings((prev) => ({
      ...prev,
      [selectedScenario.id]: { stars: formStars, comment: formComment.trim() },
    }))
      ...
      >
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Box sx={{ minWidth: 0 }}>
              <Typography variant="h6" sx={{ fontWeight: 700, color: 'text.primary' }}>
                {ratingDialogMode === 'rate' ? 'Rate this scenario' : 'Your rating'}
              </Typography>
              {selectedScenario && (
        ...
        <DialogContent sx={{ pt: 3, pb: 2, px: { xs: 2.5, sm: 3.5 } }}>
          {ratingDialogMode === 'rate' ? (
            ...
            <Box>
              {selectedScenario && getRatingForCourse(selectedScenario.id) ? (
                <>
                  <Typography variant="subtitle2" sx={{ fontWeight: 600, color: 'text.primary', mb: 1 }}>
                    Stars
                  </Typography>
                  <Rating
                    name="view-rating"
                    value={getRatingForCourse(selectedScenario.id).stars}
                    readOnly
                    size="large"
                    icon={<StarRoundedIcon sx={{ fontSize: 36 }} />}
                    emptyIcon={<StarBorderRoundedIcon sx={{ fontSize: 36 }} />}
                  ...
                  />
                  {getRatingForCourse(selectedScenario.id).comment && (
                    <>
                      <Typography variant="subtitle2" sx={{ fontWeight: 600, color: 'text.primary', mb: 1 }}>
                        Your comment
                      </Typography>
                ...
                <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                  You haven&apos;t rated this scenario yet. Use &quot;Rate this&quot; to add your rating and comment.
                </Typography>
              )}
            </Box>
          )}
