...
function AdminCoursesDifficultyLevel() {
  ...
  const fetchLevels = async (opts = {}) => {
    ...
    try {
      const { ok, data } = await apiClient(`/difficulty-levels?${params.toString()}`, 'GET')
      if (!ok || !data?.success) {
        const message =
          data?.errors && typeof data.errors === 'object'
            ? Object.values(data.errors).flat().join(' ')
            : data?.message
        setListError(message || 'Unable to load difficulty levels.')
        return
      }

      const list = data.data?.difficulty_levels || []
  ...
  useEffect(() => {
    fetchLevels({ applyFilters: false, targetPage: 1, targetPerPage: rowsPerPage })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  const handleSearch = () => {
    setPage(0)
    fetchLevels({ applyFilters: true, targetPage: 1 })
  }
  const handleReset = () => {
    setSearch('')
    setStatusFilter('')
    setPage(0)
    fetchLevels({ applyFilters: false, targetPage: 1 })
  }
  ...
  const handleDialogSubmit = async (e) => {
    ...
    const path =
      dialogState.mode === 'edit'
        ? `/difficulty-levels/${dialogState.id}`
        : '/difficulty-levels'
    ...
    try {
        ...
        if (!errors.name && !errors.status) {
          const serverMessage =
            data?.errors && typeof data.errors === 'object'
              ? Object.values(data.errors).flat().join(' ')
              : data?.message
          setDialogNameError(serverMessage || 'Unable to save difficulty level. Please try again.')
        }
        return
      }
      ...
      showToast(
        dialogState.mode === 'edit'
          ...
          : 'Difficulty level added successfully.',
        'success',
      )
      handleDialogClose()
      fetchLevels({ applyFilters: !!(search || statusFilter), targetPage: serverPage })
    } catch {
  ...
  const handleDelete = async (row) => {
    ...
    try {
      const { ok, data } = await apiClient(`/difficulty-levels/${row.id}`, 'DELETE')
      if (!ok || !data?.success) {
        const message =
          data?.errors && typeof data.errors === 'object'
            ? Object.values(data.errors).flat().join(' ')
            : data?.message
        showToast(message || 'Unable to delete difficulty level.', 'error')
        return
      }
      showToast('Difficulty level deleted successfully.', 'success')
      fetchLevels({ applyFilters: !!(search || statusFilter), targetPage: serverPage })
    } catch {
  ...
  const handleRestore = async (row) => {
    ...
    setRowActionLoading((prev) => ({ ...prev, [row.id]: true }))
    try {
      const { ok, data } = await apiClient(`/difficulty-levels/${row.id}/restore`, 'POST')
      if (!ok || !data?.success) {
        const message =
          data?.errors && typeof data.errors === 'object'
            ? Object.values(data.errors).flat().join(' ')
            : data?.message
        showToast(message || 'Unable to restore difficulty level.', 'error')
        return
      }
      showToast('Difficulty level restored successfully.', 'success')
      fetchLevels({ applyFilters: !!(search || statusFilter), targetPage: serverPage })
    } catch {
      showToast('Unable to reach server. Please try again.', 'error')
    } finally {
    ...
    >
      ...
      <Box sx={{ mb: { xs: 2, sm: 3 } }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, mb: 0.25 }}>
          <TrendingUpRoundedIcon sx={{ fontSize: { xs: 28, sm: 32 }, color: ADMIN_PRIMARY }} />
      ...
      >
        ...
        >
          ...
          >
            {confirmState.mode === 'restore'
              ? 'Are you sure you want to restore this difficulty level?'
              : 'Are you sure you want to delete this difficulty level? You can restore it later from this list.'}
          </Typography>
          {confirmState.row && (
            <Typography
              variant="subtitle2"
                  ...
                  <TableRow>
                    <TableCell colSpan={3} align="center" sx={{ py: 6 }}>
                      ...
                      >
                        ...
                        >
                          Use the filters above or click &quot;Add Level&quot; to create a new difficulty level.
                        </Typography>
                      </Box>
                    </TableCell>
                  </TableRow>
              ...
              : levels.length === 0 ? (
                ...
                >
                  ...
                  >
                    ...
                    >
                      Adjust your filters or add a new difficulty level to get started.
                    </Typography>
                  </Box>
                </Paper>
              ) : levels.map((row) => (
      ...
      {/* Add/Edit Difficulty Level dialog — style matched to AdminCoursesExamType */}
      <Dialog
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 0 }}>
            ...
            <Typography variant="h6" sx={{ fontWeight: 700 }}>
              {dialogState.mode === 'edit' ? 'Edit Difficulty level' : 'Add Difficulty level'}
            </Typography>
          </Box>
          <IconButton
            size="small"
        ...
        >
          ...
          >
            ...
            {dialogState.mode === 'edit' && (
              ...
              >
                <InputLabel id="difficulty-level-status-label">Status</InputLabel>
                <Select
                  labelId="difficulty-level-status-label"
                  label="Status"
...
export default AdminCoursesDifficultyLevel
