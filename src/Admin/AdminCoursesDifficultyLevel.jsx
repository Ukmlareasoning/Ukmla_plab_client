...
const STATIC_LEVELS = [
  ...
  { id: 2, name: 'Core', status: 'Active' },
  { id: 3, name: 'Advanced', status: 'Inactive' },
]
...
function AdminCoursesDifficultyLevel() {
  ...
  const [addDialog, setAddDialog] = useState({ open: false, level: '' })

  const filtered = levels.filter((row) => {
  ...
  const handleAddDialogOpen = () => setAddDialog({ open: true, level: '' })
  const handleAddDialogClose = () => setAddDialog({ open: false, level: '' })
  const handleAddLevel = () => {
    if (!addDialog.level.trim()) return
    setLevels((prev) => [
      ...prev,
      { id: Math.max(0, ...prev.map((e) => e.id)) + 1, name: addDialog.level.trim(), status: 'Active' },
    ])
    handleAddDialogClose()
  }

  const handleDelete = (id) => {
    setLevels((prev) => prev.filter((e) => e.id !== id))
  }
  ...
  return (
    ...
    >
      ...
      {/* Add Difficulty Level dialog — style matched to AdminCoursesExamType */}
      <Dialog
        open={addDialog.open}
        ...
        >
          <Box sx={{ pt: 4, display: 'flex', flexDirection: 'column', gap: 3 }}>
            <TextField
              ...
              onChange={(e) => setAddDialog((p) => ({ ...p, level: e.target.value }))}
              fullWidth
              size="small"
