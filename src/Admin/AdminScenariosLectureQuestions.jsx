...
function AdminScenariosLectureQuestions() {
  ...
  const handleFilterChange = (value) => {
    ...
    apiClient(`/scenario-questions?${params.toString()}`, 'GET').then(({ ok, data }) => {
      if (!ok || !data?.success) {
  ...
  const handleBack = () => {
    navigate('/admin/scenarios/lectures', { state: { courseTitle, scenarioId } })
  }
  ...
  return (
    <Box sx={{ width: '100%', minWidth: 0, maxWidth: 1000, mx: 'auto', overflowX: 'hidden' }}>
      ...
      <Box sx={{ mb: { xs: 2, sm: 3 }, display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
        <IconButton onClick={handleBack} size={isMobile ? 'medium' : 'large'} sx={{ color: ADMIN_PRIMARY, bgcolor: alpha(ADMIN_PRIMARY, 0.08), borderRadius: '7px', '&:hover': { bgcolor: alpha(ADMIN_PRIMARY, 0.15) } }} aria-label="Back to scenario exams">
