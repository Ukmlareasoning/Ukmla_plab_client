...
function AdminCoursesLectures() {
  ...
  const fetchExams = async (opts = {}) => {
    ...
    params.set('mock_id', String(courseId))
    ...
    try {
      const { ok, data } = await apiClient(`/mock-exams?${params.toString()}`, 'GET')
      if (!ok || !data?.success) {
      ...
      const list = data.data?.mock_exams || []
  ...
  useEffect(() => {
    if (!courseId) {
      setListError('No mock exam selected. Please go back and select a mock exam.')
      return
    }
  ...
  const handleSaveReleaseMode = async () => {
    if (!courseId || saveReleaseModeLoading) return
    setSaveReleaseModeLoading(true)
    try {
      const { ok, data } = await apiClient('/mock-exams/release-mode', 'POST', {
        mock_id: courseId,
  ...
  const handleViewQuestions = (row) => {
    navigate('/admin/courses/lectures/questions', {
      state: { courseTitle: courseTitleFromState, lectureId: row.id, lectureNo: row.exam_no, courseId },
    })
  }
    ...
    <Box sx={{ ...keyframes, width: '100%', minWidth: 0, maxWidth: 1400, mx: 'auto', overflowX: 'hidden' }}>
      ...
      <Box sx={{ mb: { xs: 2, sm: 3 } }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
          <Button size="small" startIcon={<ArrowBackRoundedIcon />} onClick={handleBackToCourses} sx={{ color: 'text.secondary', fontWeight: 600, '&:hover': { color: ADMIN_PRIMARY, bgcolor: alpha(ADMIN_PRIMARY, 0.08) } }}>
            Back
          </Button>
        </Box>
        ...
        <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.25 }}>
          {courseTitleFromState || 'Manage mock exams'}
        </Typography>
      </Box>
                      ...
                      <TableRow>
                        <TableCell colSpan={3} align="center" sx={{ py: 6 }}>
                          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                            ...
                            <Typography variant="body2" sx={{ color: 'text.disabled', maxWidth: 280 }}>Exams are created automatically when you add a mock exam.</Typography>
                          </Box>
                        </TableCell>
                      </TableRow>
                    )
      ...
      >
        ...
        <DialogTitle component="div" sx={{ pt: { xs: 2.5, sm: 4 }, pb: 2.5, px: { xs: 2.5, sm: 3.5 }, display: 'flex', alignItems: 'center', gap: 2, borderBottom: '1px solid', borderColor: alpha(ADMIN_PRIMARY, 0.1), bgcolor: alpha(ADMIN_PRIMARY, 0.02) }}>
          ...
          <Box sx={{ minWidth: 0 }}>
            <Typography variant="h5" sx={{ fontWeight: 700, color: 'text.primary', letterSpacing: '-0.02em', lineHeight: 1.3 }}>Block / Unblock mock exams</Typography>
