...
function AdminScenariosQuestionBank() {
  ...
  const handleEdit = (row) => {
    navigate('/admin/scenarios/question-bank/add', { state: { editQuestion: row } })
  }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const url = isDeleted ? `/scenario-questions/${id}/restore` : `/scenario-questions/${id}`
    ...
    <Box sx={{ ...keyframes, width: '100%', minWidth: 0, maxWidth: 1400, mx: 'auto', overflowX: 'hidden' }}>
      ...
      <Paper elevation={0} sx={{ p: { xs: 1.5, sm: 2 }, mb: 2, borderRadius: '7px', border: '1px solid', borderColor: alpha(ADMIN_PRIMARY, 0.12), bgcolor: theme.palette.background.paper }}>
        <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: { xs: 1, sm: 2 } }}>
          ...
          />
          {/* <FormControl size="small" sx={{ minWidth: { xs: '100%', sm: 120 }, flex: { xs: '1 1 100%', sm: '0 0 auto' }, '& .MuiOutlinedInput-root': { bgcolor: theme.palette.grey[50], borderRadius: '7px' }, '& .MuiInputLabel-root.Mui-focused': { color: ADMIN_PRIMARY } }}>
            <InputLabel id="q-type-label">Question type</InputLabel>
            <Select labelId="q-type-label" value={questionTypeFilter} label="Question type" onChange={(e) => setQuestionTypeFilter(e.target.value)}>
      ...
      <Paper elevation={0} sx={{ borderRadius: '7px', border: '1px solid', borderColor: alpha(ADMIN_PRIMARY, 0.12), overflow: 'hidden', bgcolor: theme.palette.background.paper }}>
        ...
        <Box sx={{ px: { xs: 1.5, sm: 2 }, py: 1.5, borderBottom: '1px solid', borderColor: theme.palette.grey[200], display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 1 }}>
          ...
          <Button variant="contained" startIcon={<AddRoundedIcon />} onClick={() => navigate('/admin/scenarios/question-bank/add')} sx={{ bgcolor: ADMIN_PRIMARY, borderRadius: '7px', fontWeight: 600, '&:hover': { bgcolor: ADMIN_PRIMARY_DARK } }}>
                    ...
                    : questions.map((row) => {
                        ...
                        return (
                          <TableRow key={row.id} hover sx={{ '&:hover': { bgcolor: alpha(ADMIN_PRIMARY, 0.04) }, '& .MuiTableCell-body': { borderColor: theme.palette.grey[200], py: 1.5, fontSize: '0.875rem' }, opacity: isDeleted ? 0.6 : 1 }}>
                            <TableCell>
                              <Typography variant="body2" sx={{ fontWeight: 600, color: 'text.primary' }} noWrap>{row.scenario_title || '—'}</Typography>
                            </TableCell>
                            <TableCell>
          ...
          {viewDialog.row && (
            <Box sx={{ pt: 4, display: 'flex', flexDirection: 'column', gap: 3 }}>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1.5 }}>
                {viewDialog.row.scenario_title && (
                  <Box>
                    <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, display: 'block', mb: 0.5 }}>Scenario</Typography>
                    <Typography variant="body2" sx={{ fontWeight: 600, color: 'text.primary' }}>{viewDialog.row.scenario_title}</Typography>
                  </Box>
                )}
        ...
        <DialogActions sx={{ px: 3, pb: 2.5, gap: 1 }}>
          ...
          <Button
            ...
            sx={{
              ...
              fontWeight: 600,
              color: '#fff',
              bgcolor: deleteDialog.isDeleted ? theme.palette.success.main : theme.palette.error.main,
              '&:hover': { bgcolor: deleteDialog.isDeleted ? theme.palette.success.dark : theme.palette.error.dark, color: '#fff' },
              '&.Mui-disabled': { opacity: 1, color: '#fff', bgcolor: deleteDialog.isDeleted ? theme.palette.success.main : theme.palette.error.main },
            }}
          >
