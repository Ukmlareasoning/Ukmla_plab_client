...
function AdminCoursesTopicFocus() {
  ...
  const fetchTopics = async (opts = {}) => {
    ...
    try {
      const { ok, data } = await apiClient(`/topic-focuses?${params.toString()}`, 'GET')
      if (!ok || !data?.success) {
        const message =
          data?.errors && typeof data.errors === 'object'
            ? Object.values(data.errors).flat().join(' ')
            : data?.message
        setListError(message || 'Unable to load topics.')
        return
      }

      const list = data.data?.topic_focuses || []
  ...
  const handleDialogSubmit = async (e) => {
    ...
    try {
        ...
        if (!errors.name && !errors.status) {
          const serverMessage =
            data?.errors && typeof data.errors === 'object'
              ? Object.values(data.errors).flat().join(' ')
              : data?.message
          setDialogNameError(serverMessage || 'Unable to save topic / focus. Please try again.')
        }
        return
      }
      ...
      showToast(
        dialogState.mode === 'edit'
          ? 'Topic / focus updated successfully.'
          : 'Topic / focus added successfully.',
        'success',
      )
      handleDialogClose()
  ...
  const handleDelete = async (row) => {
    ...
    try {
      const { ok, data } = await apiClient(`/topic-focuses/${row.id}`, 'DELETE')
      if (!ok || !data?.success) {
        const message =
          data?.errors && typeof data.errors === 'object'
            ? Object.values(data.errors).flat().join(' ')
            : data?.message
        showToast(message || 'Unable to delete topic / focus.', 'error')
        return
      }
      showToast('Topic / focus deleted successfully.', 'success')
      fetchTopics({ applyFilters: !!(search || statusFilter), targetPage: serverPage })
    } catch {
  ...
  const handleRestore = async (row) => {
    ...
    setRowActionLoading((prev) => ({ ...prev, [row.id]: true }))
    try {
      const { ok, data } = await apiClient(`/topic-focuses/${row.id}/restore`, 'POST')
      if (!ok || !data?.success) {
        const message =
          data?.errors && typeof data.errors === 'object'
            ? Object.values(data.errors).flat().join(' ')
            : data?.message
        showToast(message || 'Unable to restore topic / focus.', 'error')
        return
      }
      showToast('Topic / focus restored successfully.', 'success')
      fetchTopics({ applyFilters: !!(search || statusFilter), targetPage: serverPage })
    } catch {
      showToast('Unable to reach server. Please try again.', 'error')
    } finally {
    ...
    >
      ...
      >
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Typography component="span" sx={{ fontWeight: 700 }}>
              {confirmState.mode === 'restore' ? 'Restore topic / focus' : 'Delete topic / focus'}
            </Typography>
          </Box>
        </DialogTitle>
        <DialogContent
        ...
        >
          ...
          >
            {confirmState.mode === 'restore'
              ? 'Are you sure you want to restore this topic / focus?'
              : 'Are you sure you want to delete this topic / focus? You can restore it later from this list.'}
          </Typography>
          {confirmState.row && (
            <Typography
              variant="subtitle2"
                  ...
                  <TableRow>
                    <TableCell colSpan={3} align="center" sx={{ py: 6 }}>
                      ...
                      >
                        ...
                        >
                          No topics / focus areas found.
                        </Typography>
                        <Typography
                          variant="body2"
                          sx={{ color: 'text.disabled', maxWidth: 320 }}
                        >
                          Use the filters above or click &quot;Add Topic&quot; to create a new topic / focus.
                        </Typography>
                      </Box>
                    </TableCell>
                  </TableRow>
              ...
              : topics.length === 0 ? (
                ...
                >
                  ...
                  >
                    ...
                    >
                      Adjust your filters or add a new topic / focus to get started.
                    </Typography>
                  </Box>
                </Paper>
              ) : topics.map((row) => (
      ...
      {/* Add/Edit Topic dialog — style matched to AdminCoursesExamType */}
      <Dialog
        ...
        >
          ...
          >
            ...
            {dialogState.mode === 'edit' && (
              ...
              >
                <InputLabel id="topic-focus-status-label">Status</InputLabel>
                <Select
                  labelId="topic-focus-status-label"
                  label="Status"
